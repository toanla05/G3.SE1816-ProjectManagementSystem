...
public class ProjectManagement {
	...
	//methods to check if the date string is correctly formatted and logically correct
	...
	//method for writing project to file
    	private void writeProject (Project project) {
        	try {
            		...
            		String result = project.getID() + "|" + project.getName() + "|" + project.getDescription() + "|" + project.getCategory() + "|" + 
	...
	//method for writing task to file
    	private void writeTask (Task task) {
        	try {
            		...
            		//Create string result to write to file
            		String result = task.getID() + "|" + task.getName() + "|" + task.getDescription() + "|" + task.parseDate(task.getStartDate()) + "|" + 
	...
	public void createProject () {
		...
		//Ask for project's name
        	...
        	//Handle error if user enter an empty string (or string that only contains space)
		...
		//Ask for each task's information
			...
			//Check if task's description is an empty string (or string that contains only space)
			...
			do {
				do {
					...
					while (parseToDate(taskStartDate) == null) {
						System.out.println("The task's start date is not formatted correctly or logically incorrect!!!");
						System.out.print("Please enter task's start date (dd/mm/yyyy): ");
						taskStartDate = sc.next();
					}
				...
				if (parseToDate(taskStartDate).before(parseToDate(startDate)) || parseToDate(taskEndDate).after(parseToDate(endDate))) {
					System.out.println("The task's duration must be between in project's duration");
				}
			} while (parseToDate(taskStartDate).before(parseToDate(startDate)) || parseToDate(taskEndDate).after(parseToDate(endDate)));
